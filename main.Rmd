...
```{r}
data %>% ggplot(aes(x=time))+geom_line(aes(y=operator))+geom_line(aes(y=wall),color="red")
```
...
data %>% mutate(hour=hour(time)) %>% group_by(hour) %>% summarise(op_avg=mean(operator),wa_avg=mean(wall)) %>% ggplot(aes(x=hour))+geom_line(aes(y=op_avg))+geom_line(aes(y=wa_avg),color="red")+geom_hline(yintercept = 14.3,color="blue")
```

zamana göre standart sapmalar nasıl değişiyor?

```{r}
data %>% mutate(hour=hour(time)) %>% group_by(hour) %>% summarise(sd=sd(total)) %>% ggplot(aes(x=hour))+geom_col(aes(y=sd))
```

toplu gösterim

```{r}
data %>% mutate(hour=hour(time)) %>% ggplot(aes(x=factor(hour),y=operator))+geom_violin(aes(fill = factor(hour)))+ geom_jitter(height = 0, width = 0.05)+geom_hline(yintercept = 14.3,color="blue")
```
